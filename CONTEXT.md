...
## Model
The model that serves a given LLM call, across four providers (**Anthropic Claude**, **Google Gemini**, **DeepSeek**, **Z.ai GLM**). Ten aliases: Claude `haiku` (default), `sonnet` (Sonnet 5), `opus` (Opus 5); Gemini `gemini-flash-lite`, `gemini-flash` (2.5), `gemini-pro` (3.1 Pro); DeepSeek `deepseek-flash`, `deepseek-pro` (V4); GLM `glm-air`, `glm`. The **root query (`QUERY`) always uses `ROOT_MODEL` (currently `gemini-2.5-flash`) and is not user-selectable**; swapping `ROOT_MODEL` to a Claude id in `apps/api/src/llm/models.ts` reverts the entire root flow — including streaming — to Anthropic with no other change (`streamAnswerQuery` dispatches on `providerNameFor(ROOT_MODEL)`). The serving model id is emitted in the `done` SSE event (`{ model: ROOT_MODEL }`) so the ✳ pill appears on the root node immediately after the stream ends, without waiting for a full session reload. **Branch calls (`DEEPER` and `ASK`) honour a single per-device "Model" tweak** in the Tweaks panel, persisted in `localStorage` under `fork.ai.tweaks`, default `haiku`. The client sends the alias, never a raw model id; the server validates it against a fixed allowlist and maps it to a concrete model id, falling back to `haiku` if absent or invalid. The Tweaks dropdown shows each model's **approximate cost ×N relative to Claude Haiku** (blended input+output). A **Guest is clamped to the mid tier within the chosen provider** — `/share/:token/*` downgrades a top-tier request (`opus`→`sonnet`, `gemini-pro`→`gemini-flash`, `deepseek-pro`→`deepseek-flash`), because Guest branches spend the owner's Credit and the owner never opted into top-tier-priced calls. `LlmService` dispatches to the right provider by model id; the serving Model is recorded on the **Node** (shown as the ✳ pill in the node header) and on the Usage Event, and determines the rate used by the Credit Multiplier formula. **Web Search**: a Gemini model uses Google Search grounding (vs Anthropic's web-search tool), mapped to the same `CitationSource` footnotes (Gemini 2.5 forbids grounding + JSON-output together, so grounded Gemini calls rely on prompt-based JSON + the `parseJson` fallback). **DeepSeek has no web search** (`supportsWebSearch` is false) — the toggle is greyed out in the UI for DeepSeek models and the flag is dropped server-side. DeepSeek is reached via its **Anthropic-compatible endpoint** (`https://api.deepseek.com/anthropic`), so it reuses `@anthropic-ai/sdk` rather than a new SDK; reasoning is disabled for fast structured-JSON extraction.

## Output Budget
The maximum length a single branch answer (`DEEPER`/`ASK`) may reach, tiered by the caller's authentication and the answer style. An authenticated caller gets a larger budget (Verbose > Sectioned); a Guest/Trial caller stays on the smallest budget, because Guest branches spend the owner's Credit. Root queries (`QUERY`) are not subject to this — they stream and degrade to partial sections instead. The budget is a server-side ceiling enforced on the non-streaming branch call; uncapping past the non-streaming safe ceiling would require streaming the branch path (see ADR-0009).

## Cut-Off
A branch answer that stopped because it reached its **Output Budget** — distinct from an **Unreadable** answer, which is content the LLM returned that could not be parsed at all. A Cut-Off is reported to the user as having hit the length limit (not as "unreadable"). An authenticated caller may Retry a Cut-Off, which re-runs the branch at double the budget; a Guest cannot Retry a Cut-Off (a same-budget retry would only cut off again).
...
## Credit Multiplier
A scalar applied on top of the raw Anthropic API cost to compute how much Credit is deducted per LLM call. Formula: `deduction = (inputTokens × inRate + outputTokens × outRate) × multiplier`, where `inRate`/`outRate` are the **per-model** token rates (Claude or Gemini) for the model that actually served the call (see Model). Configured via the `CREDIT_MULTIPLIER` env var (default `1.5`). Server-side only — not stored per-user or per-call. The per-model rate table is server-side and is the single source of truth for cost; the client-selected model never sets the price.
...
## Platform Metrics
Admin-only aggregates over the whole `forkai-main` table, served by `GET /admin/metrics` and rendered on the admin dashboard Overview. Scalars: users, sessions, nodes, revenue (Σ Payment `amountUsd`), LLM spend (Σ Usage Event `costUsd`), outstanding credit — plus a per-day `series`. **LLM spend is broken down by provider** (Claude/Gemini/DeepSeek, derived from each Usage Event's `model` via `providerNameFor`) both all-time (`llmSpendByProvider`) and per-day (`spendByProvider`); the dashboard "LLM spend" card sums the series into Today / 7-day / 30-day / All-time windows (UTC date buckets, matching how the server keys days). Because Dynamoose strips fields a model's schema doesn't declare on a cross-entity scan (see root `CLAUDE.md → Dynamoose saveUnknown`), cost and revenue are read via `usageEventModel` / `paymentModel` scans — **not** the `userMetaModel` scan, which silently returned `$0`. Cached 60s server-side; the dashboard's Refresh button passes `?fresh=1` to bypass the cache. Usage Events written before `model` was added to `UsageEventSchema` are model-less and therefore counted as Claude.
