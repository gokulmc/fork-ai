...
- The checkout's phone-field country-code dropdown **cannot** be defaulted to the user's location. Razorpay Standard Checkout exposes only `prefill.contact` (`+{code}{number}`): a code-only value (e.g. `+1`) is silently ignored (dropdown stays at the merchant default, +91), only a full number switches the country (which also prefills that number), and Razorpay caches the last contact on the device so a placeholder would persist incorrectly. Verified live 2026-06-28. Decision: no `prefill` — the dropdown defaults to +91 and the user switches it manually.
