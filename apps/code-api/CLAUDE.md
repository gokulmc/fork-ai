...
> **forkai-code fork.** This is a copy of `apps/api`, scaffolded as the backend for the `forkai-code` product pair (port 4000, table `forkai-code-main`). The Notion export, guest/share/trial mode, admin dashboard, blog, referrals, and transactional email modules present in the original `apps/api` were stripped out — see root `CLAUDE.md` → "forkai-code" for the full picture. Most of the content below still describes the shared architecture this fork inherited; treat any mention of a stripped module as historical/inapplicable here.
