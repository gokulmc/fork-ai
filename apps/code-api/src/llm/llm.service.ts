...
You MAY use GitHub-flavored markdown. The "title" should be a 5-word-max phrase capturing the topic. Escape double-quotes inside JSON strings.`;
  ...
  async mixNodes(
    ...
    // Plan mode reads source sections for implementation detail, not just gist —
    // give it a much larger per-section budget than the normal mix synthesis.
    const bodyCap = plan ? 4000 : 600;
    const nodeBlocks = sourceNodes
      .map((n, i) => {
        const sections = n.sections
          .map((s) => `  ${s.heading ? `**${s.heading}**: ` : ''}${s.body.slice(0, bodyCap)}`)
          .join('\n');
    ...
    const prompt = plan
      ? `${personaPreamble(persona)}You are a senior software engineer turning research into an actionable implementation plan for a coding agent.

Research trail (root → base node):
${trail}

The user selected ${sourceNodes.length} branch${sourceNodes.length > 1 ? 'es' : ''} as source material:

${nodeBlocks}

The user's planning request: "${question}"

Synthesize an IMPLEMENTATION PLAN grounded in the research above. Produce EXACTLY these 5 sections, in this order, with these exact headings: "Goal", "Approach", "Steps", "Files to touch", "Risks".
- "Goal": one paragraph stating what the change accomplishes.
- "Approach": the high-level technical approach and key decisions.
- "Steps": a numbered, ordered list of concrete implementation steps.
- "Files to touch": a bullet list of the files/modules likely to change and why.
- "Risks": a bullet list of what could go wrong or needs care.

${SECTIONS_SCHEMA}

You MAY use GitHub-flavored markdown. The "title" should be a 5-word-max phrase capturing the plan. Escape double-quotes inside JSON strings.`
      : `${personaPreamble(persona)}You are a synthesis research assistant. Your task is to combine insights from multiple research branches into one coherent answer.
