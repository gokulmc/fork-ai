...
`tweaks.branchModel` (persisted in `fork.ai.tweaks`) chooses the model for **branch** calls (Go Deeper / Ask AI); it's sent as `model` on the create-node payload (authed and guest). Ten options in `MODEL_OPTIONS` (`TweaksPanel.tsx`; the default is `gemini-flash-lite`, set in `TWEAK_DEFAULTS` in `App.tsx`), rendered as a `TweakSelect`: Claude `haiku`/`sonnet`/`opus` + Gemini `gemini-flash-lite`/`gemini-flash`/`gemini-pro` + DeepSeek `deepseek-flash`/`deepseek-pro` + GLM `glm-air`/`glm`. The root query uses `ROOT_MODEL` server-side (not selectable by the user).
