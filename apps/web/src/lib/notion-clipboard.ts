...
import { cleanHeading } from './utils';
...
function nodeToNBlocks(
  ...
  for (const section of node.sections) {
    ...
    if (heading) {
      if (depth === 0) {
        sectionBlocks.push({ type: 'heading_1', heading_1: { rich_text: [makeRT(heading, { color: 'blue' })], is_toggleable: false, color: 'default' } });
      } else if (depth === 1) {
        sectionBlocks.push({ type: 'heading_3', heading_3: { rich_text: [makeRT(heading)], is_toggleable: false, color: 'default' } });
      } else {
        sectionBlocks.push({ type: 'paragraph', paragraph: { rich_text: [makeRT(heading, { bold: true })], color: 'default' } });
      }
    }
...
function sectionsToHtml(
  ...
  node.sections.forEach((section, i) => {
    ...
    if (heading) {
      ...
      html += `<${htag}>${escHtml(heading)}</${htag}>\n`;
    }
    const hls = persistentHl[`${node.id}::${section.id}`] ?? [];
    let bodyHtml: string;
...
function starredNodeBlocks(
  ...
  for (const section of node.sections) {
    ...
    const heading = cleanHeading(section.heading);
    if (heading) {
      inner.push({ type: 'paragraph', paragraph: { rich_text: [makeRT(heading, { bold: true })], color: 'default' } });
    }
