...
import { Bookmark, X, FileText, Lightbulb, PageIcon, Trash } from './Icons';
...
export function NotesDrawer({ open, onClose, items, onJump, onRemove }: NotesDrawerProps) {
  const [tab, setTab] = useState<'all' | 'note' | 'callout'>('all');
    ...
    <>
      ...
      <aside className={`drawer${open ? ' open' : ''}`} aria-hidden={!open}>
        <div className="drawer-head">
          <h3><Bookmark className="ic" /> Notes &amp; Callouts</h3>
              ...
              <div key={n.id} className={`note-card ${n.kind}`}>
                ...
                <div className="meta">
                  <span className="from" onClick={() => onJump(n.nodeId)}>
                    <PageIcon className="ic" /> {n.fromTitle}
                  </span>
