...
> **forkai-code fork.** This is a copy of `apps/web`, scaffolded as the frontend for the `forkai-code` product pair (port 4001, talks to `apps/code-api` on port 4000). Blog, admin dashboard, Notion export, guest/share mode, mobile-shell install prompts, and the onboarding tour present in the original `apps/web` were stripped out — see root `CLAUDE.md` → "forkai-code" for the full picture. Most of the content below still describes the shared architecture this fork inherited; treat any mention of a stripped feature as historical/inapplicable here.
...
`tweaks.branchModel` (persisted in `fork.ai.tweaks`) chooses the model for **branch** calls (Go Deeper / Ask AI); it's sent as `model` on the create-node payload (authed and guest). Three Claude-only options in `MODEL_OPTIONS` (`TweaksPanel.tsx`, default `haiku`): `haiku`/`sonnet`/`opus` (Haiku 4.5 / Sonnet 5 / Opus 5). The root query uses `ROOT_MODEL` server-side (not selectable by the user).
