...
### Model selection & providers (Claude + Gemini + DeepSeek + GLM)

The backend supports **four LLM providers** behind a thin abstraction (`apps/api/src/llm/providers/`): Anthropic Claude, Google Gemini, DeepSeek, and Z.ai GLM. **Branch calls** (`expandSection`, `followUpFromHighlight`) take a `model` param and run on the user-selected model; the **root query always runs `ROOT_MODEL`** (fork.ai: `gemini-2.5-flash`; forkai-code: Claude Sonnet). Claude aliases resolve to the Claude 5 family (`sonnet` → Sonnet 5, `opus` → Opus 5 — both sent with `thinking: disabled`, see `isClaude5` in `models.ts`); `gemini-pro` resolves to Gemini 3.1 Pro (`thinkingLevel: LOW` on 3.x). Ten aliases — `haiku`/`sonnet`/`opus`, `gemini-flash-lite`/`gemini-flash`/`gemini-pro`, `deepseek-flash`/`deepseek-pro` (V4), `glm-air`/`glm` (GLM 4.5 Air / GLM 5.2) — resolved server-side in `models.ts` (`resolveBranchModel`, allowlist + guest clamp; `priceFor` for per-model billing; `providerNameFor` dispatches `callJson` to a provider; `supportsWebSearch` gates web search per provider). Gemini web search uses Google Search grounding (grounding + JSON mode are mutually exclusive on Gemini 2.5, so grounded calls rely on prompt + `parseJson`). **DeepSeek** is reached via its **Anthropic-compatible endpoint** (`https://api.deepseek.com/anthropic`) — it reuses `@anthropic-ai/sdk` (no extra SDK), disables reasoning, and has **no web search** (the toggle greys out in the UI and the flag is dropped in `callJson`). **GLM** is the exception that does NOT use an Anthropic-compatible surface: its built-in web-search tool only lives on Z.ai's **OpenAI-style endpoint** (`https://api.z.ai/api/paas/v4/chat/completions`), so the provider hand-rolls the request via raw `fetch` (no new SDK), the way Gemini owns its own client. GLM specifics (all verified against the live API): reasoning is force-disabled (`thinking: { type: 'disabled' }`) or it spends the whole token budget on reasoning and returns an empty answer; unlike Gemini it allows `web_search` + `json_object` together, so JSON is always forced even while searching; and the endpoint returns no structured sources, so GLM self-reports them in its JSON (`[N]` markers + a `sources` array) which `processGlmCitations` resolves into the same footnotes used for Claude/Gemini (`sanitizeGlmSources` validates the model-emitted list). GLM 5.2 is slow (~16–25s); GLM 4.5 Air (~9s) is the snappier tier and the guest clamp (`glm → glm-air`). The serving model id is persisted on the node and the Usage Event. See `CONTEXT.md → Model` and `ADR-0004`.
...
**Secrets** are read from AWS Secrets Manager in `apps/api/buildspec.yml` (`secrets-manager:` block) and injected onto the EB env via the single `update-environment` `--option-settings` call. Current secrets: `forkai/anthropic-api-key`, `forkai/notion-client-secret`, `forkai/gemini-api-key`, `forkai/deepseek-api-key`, `forkai/glm-api-key`. To add a new secret: create it in Secrets Manager (the build role has `secretsmanager:GetSecretValue` on `forkai/*`), add a `secrets-manager:` line, and append a matching `OptionName=…,Value=$…` to the same `update-environment` call (never a second call). `GEMINI_API_KEY` / `DEEPSEEK_API_KEY` / `GLM_API_KEY` config is **optional** with lazy validation in their providers, so a missing key never crashes boot — it only errors on an actual call to that provider.
...
- **GitHub connectivity is GitHub-App-only** — the classic OAuth App scaffolding (auth/callback/status/repos-via-user-token) was removed (see ADR-0002's 2026-07-20 amendment); `forkai-code-api-prod` only needs `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY_B64` / `GITHUB_APP_SLUG` on the EB env. `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET`/`GITHUB_REDIRECT_URI` are no longer read by the app at all.
